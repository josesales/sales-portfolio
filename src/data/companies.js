import TcsImg from '../assets/tcs.jpg'
import HealthSolutionsImg from '../assets/health-solutions.jpg'
import TrixlogImg from '../assets/trixlog.jpg'
import DyadImg from '../assets/dyad.jpg';
import BnbImg from '../assets/bnb.jpg';
import DataprevImg from '../assets/dataprev.jpg'


export const companies1 = [TcsImg, HealthSolutionsImg, TrixlogImg];
export const companies2 = [DyadImg, BnbImg, DataprevImg];